...
st.title("Revisión de Contratos :ledger:")
st.write(
    """Una aplicación de ejemplo de ejemplo que permite a los usuarios verificar los valores que faltan o que tienen un puntaje de extracción por debajo de un umbral.
    """
)
...
#  Parameters
# 
doc_ai_context = "tb_doc_ai.doc_ai_schema"
...
col1.metric(label="Documentos Totales", value=df_agreements.count())
col2.metric(label="Documentos que necesitan Validación", value=df_validate_docs.count())
        ...
        with col_val:
            with st.form("doc_form"):
                ...
                for col in value_dict:
                    ...
                    if not col_vals[1]:
                        st.markdown(f":red[**Valuos Faltante!**]")
                        ...
                        if col_vals[0] <= threshold_score:
                            ...
                            st.markdown(f":red[**El puntaje, {col_vals[0]}, es menor al umbral definido!**]")
                            st.checkbox("Approved", key=f"check_{approve_checkboxes}")
                            approve_checkboxes += 1
                        else:
                            st.write(col_vals[1])
                ...
                if save:
                     with st.spinner("Guardando la aprobación del documento ..."):
                        for i in range(approve_checkboxes):
                            if not st.session_state[f"check_{i}"]:
                                st.error("Necesita aprobar antes de guardar")
                                st.stop()
