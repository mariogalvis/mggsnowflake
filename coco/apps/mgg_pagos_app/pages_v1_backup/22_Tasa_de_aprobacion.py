...
with tab1:
    ...
    if kpi_df.empty or kpi_df["REGISTROS"].iloc[0] == 0:
        ...
        if mapa_df.empty:
            ...
            vol_max = float(mapa_df["VOLUMEN_F"].max()) if float(mapa_df["VOLUMEN_F"].max()) > 0 else 1

            colormap = cm.LinearColormap(
                colors=["#DE350B", "#FF8B00", "#36B37E"],
                vmin=tasa_min, vmax=tasa_max,
                caption="Tasa de aprobacion (%)",
            )
            m = folium.Map(location=[4.5, -73.5], zoom_start=5, tiles="CartoDB positron")
            for _, r in mapa_df.iterrows():
                lat, lon = float(r["LAT"]), float(r["LON"])
                tasa = float(r["TASA_F"])
                vol = float(r["VOLUMEN_F"])
                rev = float(r["REV_F"])
                depto = str(r["DEPARTAMENTO"])
                radius = max(8, (vol / vol_max) * 35)
                folium.CircleMarker(
                    location=[lat, lon],
                    radius=radius,
                    color="#11567F",
                    ...
                    fill_opacity=0.8,
                    weight=1.5,
                    popup=folium.Popup(f"<b>{depto}</b><br>Tasa: {tasa}%<br>Volumen: {int(vol):,}<br>Rev. perdido: ${rev:.0f}M COP", max_width=220),
                    tooltip=f"{depto}: {tasa}%",
                ).add_to(m)
                folium.Marker(
                    location=[lat + 0.2, lon],
                    icon=folium.DivIcon(
                        html=f'<div style="font-size:10px;font-weight:bold;color:#11567F;white-space:nowrap;text-shadow:1px 1px 2px white,-1px -1px 2px white,1px -1px 2px white,-1px 1px 2px white;">{depto}</div>',
                        icon_size=(120, 20),
                        icon_anchor=(60, 10),
                    ),
                ).add_to(m)
