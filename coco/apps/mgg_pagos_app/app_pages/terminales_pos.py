    ...
    if kpi_df.empty or kpi_df["TOTAL"].iloc[0] == 0:
        ...
        if not map_df.empty:
            ...
            if map_data:
                ...
                pdf["hover"] = pdf.apply(lambda r: f"{r['city']}<br>Terminales: {r['n']:,}<br>${r['vol_m']:.1f}M COP", axis=1)
                deck = colombia_scatter_map(pdf, colorbar_title="Terminales")
                st.pydeck_chart(deck, key="ter_map")
