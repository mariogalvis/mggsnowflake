...
COLORS = ["#2D9B2D", "#6ABF4B", "#F5D63D", "#F5A623", "#E63946", "#1B8C1B", "#8BC34A", "#D32F2F"]


@st.cache_data(ttl=300, show_spinner=False)
def _get_ai_diagnostic(title, subtitle, kpi_summary):
    prompt = f"""Eres un analista senior de fintech y pagos digitales en Colombia. Analiza estos KPIs del modulo "{title}" ({subtitle}).
...
def render_page(config):
    ...
    with tab_objs[0]:
        if date_col:
            ...
            cols_f = st.columns(max(min(n_filters, 6), 1))
            clauses = ["1=1"]

        filter_selections = {}
        for i, fc in enumerate(filter_cols):
            col_idx = min(i if not date_col else i + 1, len(cols_f) - 1)
                ...
                try:
                    ...
                    if not t_df.empty:
                        ...
                        fig_t.add_trace(go.Scatter(
                            ...
                            fill="tozeroy", fillcolor="rgba(45,155,45,0.08)",
                            line=dict(color="#2D9B2D", width=3), marker=dict(size=8),
                            text=[f"{v:.1f}" for v in y1], textposition="top center",
                            textfont=dict(size=10, color="#2D9B2D"),
                        ))
                        if len(t_df.columns) > 2 and len(trend_cols) > 1:
                            ...
                            fig_t.add_trace(go.Scatter(
                                ...
                                line=dict(color="#F5A623", width=2, dash="dash"), marker=dict(size=6),
                                yaxis="y2",
                            ))
                            fig_t.update_layout(
                    ...
                    st.error(f"Error en tendencia: {e}")
                    ...
                    if not tm_df.empty:
                        fig_tree = go.Figure(go.Treemap(
                            ...
                            marker=dict(
                                ...
                                colorscale=treemap_config.get("colorscale", [[0, "#E63946"], [0.25, "#F5A623"], [0.5, "#F5D63D"], [0.75, "#6ABF4B"], [1, "#2D9B2D"]]),
        ...
        if geo_query and geo_config:
            try:
                ...
                if not geo_df.empty:
                    city_col = geo_config.get("city_col", geo_df.columns[0])
                    color_col = geo_config.get("color_col", geo_df.columns[-1])
                    size_col = geo_config.get("size_col", color_col)
                    ...
                    deck = colombia_scatter_map(
                        ...
                        colorscale=geo_config.get("colorscale", ["#E63946", "#F5A623", "#F5D63D", "#6ABF4B", "#2D9B2D"]),
                    )
