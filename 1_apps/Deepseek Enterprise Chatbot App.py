...
st.title("Asistente Empresarial Inteligente")
...
def clear_chat_history():
    st.session_state.messages = [{"role": "assistant", "content": "¿Cómo te puedo asistir hoy?"}]
...
if prompt := st.chat_input():
    ...
    if st.session_state.messages[-1]["role"] != "assistant":
        with st.chat_message("assistant"):
            status_container = st.status("Pensando ...", expanded=True)
            ...
            status_container.update(label="Razonamiento", state="complete", expanded=False)
